...
const NodeSpecSchema = z.lazy(() => z.object({
...
function flattenNode(node, commands) {
    ...
    if (node.children) {
        for (const child of node.children) {
            const childWithParent = { ...child, parentId: child.parentId ?? node.id };
...
// render_wireframe
server.tool('render_wireframe', 'Accepts a full WireframeSpec JSON object describing pages, frames, text, rectangles, and Carbon components. Breaks the spec into individual commands and queues them all for the Figma relay plugin to execute.', {
...
// queue_command
server.tool('queue_command', 'Queues a single raw command for the Figma relay plugin. Must include a "type" field (create_file, create_frame, create_text, create_rectangle, import_carbon_component, set_fill, set_layout).', {
...
// get_queue_status
server.tool('get_queue_status', 'Returns pending/done/total counts for the command queue. Use to poll until pending reaches 0.', {}, async () => {
    const pending = queue.filter(q => q.status === 'pending').length;
    const done = queue.filter(q => q.status === 'done').length;
    return {
        content: [
...
// clear_queue
server.tool('clear_queue', 'Removes all done commands from the queue. Call after the Figma plugin has executed a batch.', {}, async () => {
...
server.tool('list_carbon_components', 'Returns the registry of supported Carbon Design System component names. Use these names in carbonComponent fields of a WireframeSpec.', {}, async () => {
...
server.tool('create_frame', 'Queue a single frame creation. Convenience wrapper around queue_command for create_frame.', CreateFrameInputSchema.shape, async (input) => {
    const item = enqueue({
        type: 'create_frame',
        name: input.name,
        x: input.x,
...
server.tool('create_text', 'Queue a single text node creation. Convenience wrapper around queue_command for create_text.', CreateTextInputSchema.shape, async (input) => {
    const item = enqueue({
        type: 'create_text',
        text: input.text,
        x: input.x,
...
server.tool('import_carbon_component', 'Queue a Carbon component import by human-readable name (e.g. "Button/Primary"). Looks up the Figma component key from the registry.', ImportCarbonInputSchema.shape, async (input) => {
    ...
    const result = { queued: 1, id: item.id, componentName: input.componentName };
    if (!key) {
